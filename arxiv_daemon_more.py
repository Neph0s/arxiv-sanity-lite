...
def get_oldest_paper_index(pdb):
    return len(pdb)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--num', type=int, default=200000, help='up to how many papers to fetch')
    ...
    start = get_oldest_paper_index(pdb)

    for k in range(start, start + args.num, 100):
        ...
        if k % 2000 < 100:
            logging.info(f"k={k}, total_updated={total_updated}, zero_updates_in_a_row={zero_updates_in_a_row}")
            time.sleep(3600)
